...
class EmbeddingsGenerator:
    ...
    def _average_pool(self, last_hidden_states: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        embeddings = last_hidden.sum(dim=1) / attention_mask.sum(dim=1)[..., None].clamp(min=1e-9)
    ...
    def __call__(self, input_texts: List[str], batch_size: int = 64) -> Tensor:
        ...
        sentences_lengths = np.array([len(sentence.split()) for sentence in input_texts])
        sorted_indices = np.argsort(sentences_lengths)
        sorted_sentences = input_sentences[sorted_indices].tolist()

        sorted_inputs_embeddings = torch.zeros((n_input_texts, self.embedding_size), dtype=float16)
...
def _generate_embeddings(loaded_df, embeddings_generator):
    input_text = [f"passage: {' '.join(sentence)}" for sentence in loaded_df["Sentences Groups"].tolist()]
